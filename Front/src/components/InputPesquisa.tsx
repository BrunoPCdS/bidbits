...
export function InputPesquisa({ setLeiloes }: InputPesquisaProps) {
    ...
    async function enviaPesquisa(data: Inputs) {
        ...
        const filtrados = dados.filter((item: LeilaoType) => {
            ...
            return nome.includes(termo) || consoleNome.includes(termo) || midiaNome.includes(termo) || marcaConsole.includes(termo) || marcaMidia.includes(termo)
        })
    ...
    return (
        <div className="flex mx-auto max-w-5xl mt-3">
            <form className="flex-1" onSubmit={handleSubmit(enviaPesquisa)}>
                ...
                <div className="relative">
                    ...
                    <input type="search" id="termo" className="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Informe nome, console, midia ou marca" required
                        {...register('termo')} />
            ...
            <button type="button" className="ms-3 mt-2 focus:outline-none text-white bg-yellow-500 hover:bg-yellow-800 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-yellow-600 dark:hover:bg-yellow-700 dark:focus:ring-yellow-900"
